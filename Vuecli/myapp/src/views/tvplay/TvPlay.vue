...
export default {
  ...
  methods: {
    //添加滚动监听
    listenScroll() {
      let htmlDom = document.documentElement;//获取到HTML元素
      window.onscroll = () => {
        let hHeight = htmlDom.clientHeight; //视口高度 - 当前元素的高度
        let scrollTop = htmlDom.scrollTop; //获取去滚动多少距离 - 具顶部多高
        let scrollHeight = htmlDom.scrollHeight; //可滚动区域的高度
        // console.log(hHeight, scrollTop, scrollHeight);
        if (hHeight + scrollTop + 10 >= scrollHeight) {//判断是否滚动到底
          this.getData();
        }
      };
    },
    //跳转至详情页
    goDetail(id){
      this.$router.push("/tvdetail/"+id)//路由传参-URL传参
    },
    ...
    getData() {
      ...
      if (this.isContinue && this.total>this.start) {//判断访问请求是否许可
        ...
        axios.get(birdUrl + requestUrl).then(res => {
          ...
          this.start += 10;
          this.isContinue = true;//成功赋值后将请求状态改为true
        });
      }
    }
  },
