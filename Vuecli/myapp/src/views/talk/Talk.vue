...
export default {
  ...
  methods: {
    ...
    getTimeStamp() {
      ...
      timer = timer.toString().substr(0, 10); //通过字符串截取将毫秒级变成秒级
    ...
    getSign(obj) {
      ...
      arr.map(function(val) {
        str += `${val}=${encodeURI(obj[val])}&`; //js语法 encodeURI 把字符串转成URL
      });
      //第三步 将应用密钥以app_key为键名，组成URL键值拼接到字符串T末尾
      str += "app_key=ZUI2YkWfQ1nvgPW9";
      //第四步 对字符串进行MD5运算，将得到的MD5值所有字符转换成大写
      str = md5(str).toUpperCase(); //toUpperCase 字符串升序
      // 第五步返回结果
      return str;
    },
